...
-- Notification-driven follow-back CTA needs actor visibility and outgoing-request state.
...
as $$
  with scoped as (
    select
      ...
      coalesce(p.account_visibility, 'private') as actor_account_visibility,
  ...
  select
    ...
    exists (
      ...
      where fr.requester_id = auth.uid()
        and fr.target_id = s.actor_user_id
        and fr.status = 'pending'
